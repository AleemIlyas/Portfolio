...
export default function Project(){
    ...
    useEffect(()=>{
            ...
            adminInstance.get('/getProjects' , { signal : abortController.signal })
            .then(res=>{
                setLoading(false)
                setData(res.data)
            })
                       ...
                       return(
                        ...
                        <div className={styles.box}>
                            <div className={styles.content}>
                                <img src={`http://localhost:5500/Images/${item.image}`} alt={item.title} loading='lazy' />
                                <h3> {item.title} </h3><br/> 
                                <p>{item.description.slice(0,150)}</p>
                              <a href={item.link} target="_blank" >Go To App</a>
